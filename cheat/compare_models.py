...
def load_original_model(file_path="model.txt"):
...
def main():
    try:
        ...
        original_params = load_original_model()
        
        print("Parâmetros dos modelos:")
        print(f"scikit-learn: theta0={sklearn_params['theta0']}, theta1={sklearn_params['theta1']}")
        print(f"Implementação: theta0={original_params['theta0']}, theta1={original_params['theta1']}")
        ...
        original_predictions = [predict_price(km, original_params) for km in X]
        ...
        print(f"Implementação MSE: {original_mse:.2f}")
        ...
        original_pred_sorted = np.array(original_predictions)[sort_idx]
        
   
        plt.plot(X_sorted, sklearn_pred_sorted, 'r-', linewidth=2, label='scikit-learn')
        plt.plot(X_sorted, original_pred_sorted, 'g--', linewidth=2, label='Implementação')
        ...
        for km in sample_km:
            ...
            my_price = predict_price(km, my_params)
            
            print(f"Quilometragem: {km} km")
            print(f"  scikit-learn: {sklearn_price:.2f} €")
            print(f"  Implementação: {my_price:.2f} €")
            print(f"  Diferença: {abs(sklearn_price - my_price):.2f} €")
